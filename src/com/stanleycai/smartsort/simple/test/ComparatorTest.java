...
import com.stanleycai.smartsort.simple.SimpleFeedbackComparator;
...
public class ComparatorTest {
    ...
    private void testSingleUser(User user) {
        Arrays.sort(items, new SimpleFeedbackComparator(user));
        printSingleUserWithItems(user, items);
        for (int w = 0; w < 100; ++w) {
            user.update(items[RANDOM.nextInt(items.length)], RANDOM.nextBoolean());
            Arrays.sort(items, new SimpleFeedbackComparator(user));
            printSingleUserWithItems(user, items);
        }
    }
